...
def submit_order_from_market_data(
    base_url: str,
    investor: dict,
    instrument: dict,
    side: str,
...
def get_order_status(base_url: str, investor: dict, orders: list):
    ...
    response = requests.post(
        base_url + "/order_status_request", json=order_status_request
    )
    response_dict = jwt.decode(response.text, investor["secret"], algorithms="HS256")
    print("printing orders for investor")
...
if __name__ == "__main__":
    ...
    for instrument, instrument_market_data in market_data.items():
        ...
        passive_order_response = submit_order_from_market_data(
            base_url,
            investor_1,
            json.loads(instrument),
            "1",
            instrument_market_data["open_price"],
            100,
        )
        print("Passive Order Response:")
        print_json(passive_order_response)
        time.sleep(1)
        aggressive_order_response = submit_order_from_market_data(
            base_url,
            investor_2,
            json.loads(instrument),
            "2",
    ...
    order_status = get_order_status(
        base_url, investor_1, investor_orders["data"]["orders"]
    )
